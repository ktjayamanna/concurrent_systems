...
# risk_1: What if speculative predictor had varied accuracy? ───────────────────────────────
scenarios = [0.0, 0.25, 0.50, 0.75, 1.00]
labels_sc = ["0%", "25%", "50%", "75%", "100%"]

t_pred_s_mean = t_pred_s.mean() * 1000   # ms
t_pred_c_mean = t_pred_c.mean() * 1000
t_exec_s_mean_ms = t_exec_s_mean * 1000
t_exec_c_mean_ms = t_exec_c_mean * 1000

baseline_s = t_pred_s_mean + t_exec_s_mean_ms   # 100%
baseline_c = t_pred_c_mean + t_exec_c_mean_ms

# speculative critical path = t_pred + (1-h)*t_exec
# (on a hit: exec already done; on a miss: exec runs after pred)
...
for ax, sp_pct, sv_pct, t_p, t_e, baseline, title, c in [
    ...
    # Stacked bar: t_pred portion | t_exec remaining | t_exec saved
    ...
    for i, (sp, sv, h) in enumerate(zip(sp_pct, sv_pct, scenarios)):
        ...
        # stack: t_pred (bottom) | exec_remaining | exec_saved
...
# Risk 2: What if speculative predictor had varied speed? —
...
for ax, rows_mean_p, rows_mean_e, label, c in [
    ...
    t_p  = rows_mean_p
    t_e  = rows_mean_e
    # Speculative predictor speed assumptions (fraction of t_pred)
    speed_labels  = ["t_pred_spec = t_pred/5\n(5× faster)", "t_pred/10\n(10× faster)",
                     "t_pred/20\n(20× faster)", "t_pred/50\n(50× faster)"]
    speed_fracs   = [1/5, 1/10, 1/20, 1/50]
    y_positions   = [3, 2, 1, 0]   # one row per speed assumption

    bar_height = 0.55

    # Row 0: current sequential (reference)
    ...
    # Rows 1-4: speculative scenarios
    for y, frac, slabel in zip(y_positions, speed_fracs, speed_labels):
        t_ps = t_p * frac   # speculative predictor time
        spec_done_at = t_ps + t_e   # when speculative thread finishes
        # show: real LLM bar (full width), spec predictor + exec below
        ax.barh(y, t_p, left=0, height=bar_height, color=c, alpha=0.30)  # real LLM (faded)
    ...
    # vertical line at t_pred (when real LLM confirms)
